...
async def async_pic_download(async_client: httpx.AsyncClient,
    ...
    for url, file_name in zip(img_url_list, img_file_name_list):
        task = asyncio.create_task(_async_pic(async_client, url, file_name, download_path, request_headers, max_attempt))
...
async def _async_pic(async_client: httpx.AsyncClient, img_url: str, file_name: str, download_path: str, request_headers=None, max_attempt=3):
    pic = None
    attempt_num = 0
